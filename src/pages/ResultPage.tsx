...
import { UserEntryState, SessionState, MatchState, type Phase, type Match, type Round } from '../gql/graphql';
...
export const ResultPage: React.FC = () => {
  ...
  function getPlayerIdByIndex(index: number | undefined): string | undefined {
    if (index !== undefined) {
      return data?.userEntries?.[index]?.id;
    }
  ...
  function getGloveByIndex(playerIndex: number, gloveIndex: number): string | undefined {
    if (playerIndex !== -1 && gloveIndex !== -1) {
      return data?.userEntries?.[playerIndex]?.initialGloves?.[gloveIndex];
    }
  ...
  function getWinner(): string | undefined {
    if (!data?.userEntries) return undefined;
    const winner = data.userEntries.find(userEntry => userEntry?.state === UserEntryState.Won);
      ...
      <div key={index} className='mb-2 border border-gray-400 rounded-lg'>
        <div className='flex justify-center border-b border-gray-400 text-sm p-1'>
          <div className='flex items-center'>
            {match.state === MatchState.Ended && match.winner === match.userEntryIndices?.[0] && (
              <Crown className='w-4 h-4 text-yellow-400 ml-1' />
            )}
            <AddressDisplay address={getPlayerIdByIndex(match.userEntryIndices?.[0])} type='user'/>
          </div>
          <Swords className='w-4 h-4 mt-0.5 mx-1' />
          <div className='flex items-center'>
            <AddressDisplay address={getPlayerIdByIndex(match.userEntryIndices?.[1])} type='user'/>
            {match.state === MatchState.Ended && match.winner === match.userEntryIndices?.[1] && (
              <Crown className='w-4 h-4 text-yellow-400 ml-1' />
            )}
          </div>
        </div>
  ...
  const renderRound = (round: Round, index: number, match: Match) => {
    const submission1 = round.player1?.submission !== undefined && round.player1?.submission !== null ? round.player1.submission : -1;
    const submission2 = round.player2?.submission !== undefined && round.player2?.submission !== null ? round.player2.submission : -1;
    
    const player1Index = match.userEntryIndices?.[0] !== undefined && match.userEntryIndices?.[0] !== null ? match.userEntryIndices[0] : -1;
    const player2Index = match.userEntryIndices?.[1] !== undefined && match.userEntryIndices?.[1] !== null ? match.userEntryIndices[1] : -1;
    ...
    let player1Health: number = round.player1?.healthPoint || 0;
    let player2Health: number = round.player2?.healthPoint || 0;
        ...
        <div className='w-24 flex items-center justify-center'>
          <div className='flex items-center gap-4'>
            <div className='flex items-center w-12 justify-end'>
              {round.winner !== undefined && round.winner >= 0 && round.winner === match.userEntryIndices?.[0] && (
            ...
            <div className='flex items-center w-12'>
              ...
              {round.winner !== undefined && round.winner >= 0 && round.winner === match.userEntryIndices?.[1] && (
                <Crown className='w-4 h-4 text-yellow-400 ml-1' />
              )}
            </div>
          </div>
      ...
      <div className="flex flex-col justify-between items-center text-center p-6 h-full">
        <div className="mb-4 flex-grow">
          ...
          <p className='text-xl mb-2'>
            {t('ui:participants')}:&nbsp;{data.userEntries?.length}
          ...
          {showParticipants && (
            <div className="border border-gray-400 rounded-lg p-4 mt-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {data.userEntries?.map((player, index) => (
          ...
          {data.state === SessionState.Ended ?
            ...
            <p className='text-xl mt-2 mb-4'>{t('ui:playersLeft')}:&nbsp;{data.userEntries?.filter(player => player?.state !== UserEntryState.Lose).length}</p>}
